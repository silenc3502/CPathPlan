...
int main() {
  ...
  h.onMessage([&veh, &map_waypoints_x,&map_waypoints_y,&map_waypoints_s,&map_waypoints_dx,&map_waypoints_dy](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length,
                     uWS::OpCode opCode) {
          ...
          // j[1] is the data JSON object
            ...
            // Vehicle class requires s,s_d,s_dd,d,d_d,d_dd - in that order
					...
					if (subpath_size < 4) {
						...
						vector<double> s_dot_coeffs = veh.differentiate_coeffs(veh.s_traj_coeffs);			
						vector<double> d_dot_coeffs = veh.differentiate_coeffs(veh.d_traj_coeffs);			
						vector<double> s_ddot_coeffs = veh.differentiate_coeffs(s_dot_coeffs);			
						vector<double> d_ddot_coeffs = veh.differentiate_coeffs(d_dot_coeffs);
						eval_time = (NUM_PATH_POINTS - subpath_size) * PATH_DT;
					  pos_s2 = veh.evaluate_coeffs_at_time(veh.s_traj_coeffs, eval_time);		
						pos_d2 = veh.evaluate_coeffs_at_time(veh.d_traj_coeffs, eval_time);		
						s_dot2 = veh.evaluate_coeffs_at_time(s_dot_coeffs, eval_time);			
						d_dot2 = veh.evaluate_coeffs_at_time(d_dot_coeffs, eval_time);			
						s_ddot2 = veh.evaluate_coeffs_at_time(s_ddot_coeffs, eval_time);			
						d_ddot2 = veh.evaluate_coeffs_at_time(d_ddot_coeffs, eval_time);		
					...
					veh.s    = pos_s;           // s position
					veh.s_d  = s_dot;           // s dot - velocity in s
					veh.s_dd = s_ddot;          // s dot-dot - acceleration in s
					veh.d    = pos_d;           // d position
					veh.d_d  = d_dot;           // d dot - velocity in d
veh.d_dd = d_ddot;
...
if (car_just_ahead) cout << "CAR JUST AHEAD!!!" << endl;
            ...
            veh.update_available_states(car_to_left, car_to_right);

					// // DEBUG
					// cout << "available states: "; 
					// for (auto st: veh.available_states) cout << st << " ";
					// cout << endl; 

					vector<vector<double>> best_frenet_traj, best_target;
					double best_cost = 999999;
					string best_traj_state = "";
					for (string state: veh.available_states) {
        		vector<vector<double>> target_s_and_d = veh.get_target_for_state(state, predictions, duration, car_just_ahead);
            ...
            //     if (j != 2) cout << ", ";
						...
						vector<vector<double>> possible_traj = veh.generate_traj_for_target(target_s_and_d, duration);
